    ...
    <template v-if="editedItem">
      ...
      >
        <template v-for="key in Object.keys(editedItem)" :key="key">
          <div class="my-3">
            <va-input v-model="editedItem[key]" :label="key" />
          </div>
        </template>
      </va-modal>
    </template>
...
import { DataTableColumnSource, VaDataTable } from "vuestic-ui";
...
const createItem = ref<TableAddItem[]>([
  ...
  { key: "hq_address", editable: true, value: "" },
  { key: "logo", editable: true, value: "" },
...
const items = useState<Airline[]>("sirlines", () => []);
...
async function handleAdd() {
  try {
    const response = await useAuthApi<Airline>(
      "/api/airlines",
      "post",
...
function handleEdit(item: Airline) {
...
async function editItem() {
  try {
    const response = await useAuthApi<Airline>(
      `/api/airlines/${editedItemId.value}`,
      "put",
      editedItem.value
    );
...
function handleDelete(item: Airline) {
  showDeleteModal.value = true;
  deleteItemId.value = item.id;
}

async function deleteItem() {
  await useAuthApi(`/api/airlines/${deleteItemId.value}`, "delete");
...
onMounted(async () => {
  items.value = await useAuthApi<Airline[]>("/api/airlines", "get");
  loading.value = false;
});
</script>
